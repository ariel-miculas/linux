...
impl Rootfs {
    ...
    pub(crate) fn from_capnp(reader: crate::manifest_capnp::rootfs::Reader<'_>) -> Result<Self> {
        ...
        for blobref in metadatas.iter() {
            metadata_vec.push(BlobRef::from_capnp(blobref)?, GFP_KERNEL)?;
        }
    ...
    pub(crate) fn size(&self) -> Result<i64> {
        match &self.mode {
            ...
            InodeMode::Lnk => self
                .additional
                .as_ref()
                .and_then(|inode_additional| inode_additional.symlink_target.as_ref())
                .and_then(|symlink_target| symlink_target.len().try_into().ok())
                .ok_or_else(|| WireFormatError::from_errno(ENOENT)),
            _ => todo!(),
        }
    }
    ...
    fn from_capnp(reader: metadata_capnp::inode::mode::Reader<'_>) -> Result<Self> {
        match reader.which() {
            ...
            Ok(metadata_capnp::inode::mode::File(reader)) => {
                ...
                for chunk in r.iter() {
                    chunks.push(FileChunk::from_capnp(chunk)?, GFP_KERNEL)?;
                }
            ...
            Ok(metadata_capnp::inode::mode::Dir(reader)) => {
                ...
                for entry in r.get_entries()?.iter() {
                    ...
                    let dir_entry =
                        Vec::from_iter_fallible(entry.get_name()?.iter().cloned(), GFP_KERNEL)?;
                    entries.push(
                        DirEnt {
                            ino,
                            name: dir_entry,
...
impl Xattr {
    pub(crate) fn from_capnp(reader: metadata_capnp::xattr::Reader<'_>) -> Result<Self> {
        let key = Vec::from_iter_fallible(reader.get_key()?.iter().cloned(), GFP_KERNEL)?;
        let val = Vec::from_iter_fallible(reader.get_val()?.iter().cloned(), GFP_KERNEL)?;
        Ok(Xattr { key, val })
    }
}
...
impl MetadataBlob {
    pub(crate) fn new(f: file::RegularFile) -> Result<MetadataBlob> {
        ...
        let segments = ::capnp::serialize::NoAllocBufferSegments::from_buffer(
            metadata_buffer,
            unlimited_reads,
        )?;
...
impl fmt::Display for Digest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut hex_string =
            Vec::from_iter_fallible(encode_hex_iter(&self.underlying()), GFP_KERNEL)
                .map_err(|_| fmt::Error)?;
        // append NUL character
        hex_string.push(0, GFP_KERNEL).map_err(|_| fmt::Error)?;
...
impl TryFrom<&CStr> for Digest {
    ...
    fn try_from(s: &CStr) -> kernel::error::Result<Self, Self::Error> {
        ...
        hex::decode_to_slice(s.as_bytes(), &mut digest)?;
        Ok(Digest(digest))
    }
}
